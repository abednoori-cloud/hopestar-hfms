...
public interface StudentContractRepository extends JpaRepository<StudentContract, Long> {
    ...
    /**
     * Top-N contracts in the given status with a positive remaining balance
     * (USD-equivalent contract value minus USD-equivalent posted payments),
     * computed, filtered, and sorted entirely in one query -- each row is
     * {@code Object[]{StudentContract contract, BigDecimal remainingBalanceUsd}}.
     * {@code student} is eagerly fetched so displaying the student's name/
     * code per row never triggers a lazy-load per row. Replaces the former
     * "load a candidate pool, then run one remaining-balance query per
     * contract" approach used by the Dashboard's outstanding-receivables
     * widget (an N+1), using the same correlated-subquery shape as {@link
     * #countDistinctStudentsWithOutstandingBalance} -- just repeated across
     * the select/filter/order clauses since JPQL cannot re-use a computed
     * select-item alias in {@code WHERE}/{@code ORDER BY}.
     */
    @Query("SELECT c, c.usdEquivalentAmount - COALESCE((SELECT SUM(p1.usdEquivalentAmount) FROM StudentPayment p1 "
            ...
            + "FROM StudentContract c JOIN FETCH c.student "
