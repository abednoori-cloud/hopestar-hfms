...
/**
 * One repayment posted against an {@link EmployeeLoan} -- the auditable
 * history behind {@link EmployeeLoan#getRemainingBalance()}. Each row
 * has its own {@link #currency}/{@link #exchangeRateToUsd}/{@link
 * #usdEquivalentAmount} independent of the parent loan's, exactly like a
 * {@code StudentPayment} versus its {@code StudentContract}.
 * <p>
...
@AllArgsConstructor
@SuperBuilder
@ToString(exclude = {"loan", "paymentMethod", "ledgerTransaction"})
...
public class LoanRepaymentSchedule extends BaseEntity {
    ...
    @JoinColumn(name = "loan_id", nullable = false, foreignKey = @ForeignKey(name = "fk_loan_repayments_loan"))
    ...
    /** {@code amount * exchangeRateToUsd}, computed by {@code EmployeeLoanServiceImpl}. */
    @NotNull
    ...
    @NotNull
    @ManyToOne(fetch = FetchType.LAZY, optional = false)
    @JoinColumn(name = "payment_method_id", nullable = false, foreignKey = @ForeignKey(name = "fk_loan_repayments_payment_method"))
    private PaymentMethod paymentMethod;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "ledger_transaction_id", foreignKey = @ForeignKey(name = "fk_loan_repayments_ledger_transaction"))
